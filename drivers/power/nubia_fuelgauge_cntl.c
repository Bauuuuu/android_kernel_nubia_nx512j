...
static int  nubia_update_batt_params(struct nubia_fg_cntl *batt_cntl)
{
	...
	batt_cntl->nubia_update_fg_data(batt_cntl);
...
static void nubia_batt_worker(struct work_struct *work)
{
	...
	if( pbatt->batt_status == POWER_SUPPLY_STATUS_FULL
	...
	if(((pbatt->batt_mv > batt_cntl->low_vol_thrhld+20) || pbatt->usb_in) 
		    && wake_lock_active(batt_cntl->soc_wlock))
		wake_unlock(batt_cntl->soc_wlock);
